...
export const appAndRendererSharedConfig = () =>
  defineConfig({
    dev: {
      progressBar: true,
      writeToDisk: true,
...
export const rspackViewerConfig = (config, { appendPlugins, addRules, rspack }: ModifyRspackConfigUtils) => {
  appendPlugins(
    new rspack.NormalModuleReplacementPlugin(/data|prismarine-physics/, resource => {
      let absolute: string
  ...
  config.ignoreWarnings = [/the request of a dependency is an expression/, /Unsupported pseudo class or element: xr-overlay/]
