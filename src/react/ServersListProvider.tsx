...
const Inner = ({ hidden, customServersList }: { hidden?: boolean; customServersList?: string[] }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const newIndex = e.key === 'ArrowUp' ? Math.max(0, selectedIndex - 1) : Math.min(serversListProvided.length - 1, selectedIndex + 1)
  ...
  const serversListSorted: Array<StoreServerItem & { index: number }> = useMemo(
    () =>
      [
        ...serversListProvided,
        ...(customServersList ? [] : (miscUiState.appConfig?.promoteServers ?? [])).map(
          (server): StoreServerItem => ({
            ip: server.ip,
            name: server.name,
  ...
  const editModalJsx = isEditScreenModal ? (
    <AddServerOrConnect
      ...
      initialData={
        !serverEditScreen || serverEditScreen === true
          ? {
              ip: quickConnectIp
            }
          : serverEditScreen
  ...
  const serversListJsx = (
    <ServersList
      ...
      onProfileClick={async () => {
        const username = await showOptionsModal(
          'Select authenticated account to remove',
          authenticatedAccounts.map(a => a.username)
        )
        if (!username) return
