...
const GroupHeader = ({ name, count, expanded, onToggle }: { name: string; count: number; expanded: boolean; onToggle: () => void }) => {
  return (
    <div className={styles.world_root} style={{ background: 'none', cursor: 'pointer', height: 'auto', fontSize: '8px' }} onClick={onToggle}>
...
const World = ({
  ...
  elemRef,
  offline
}: WorldProps & { ref?: React.Ref<HTMLDivElement> }) => {
  ...
  return (
    ...
    >
      ...
      <div className={styles.world_info}>
        ...
        {formattedTextOverride ? (
          <div className={styles.world_info_formatted}>
            <MessageFormattedString message={formattedTextOverride} />
          </div>
        ) : (
          <>
            <div className={styles.world_info_description_line}>
    ...
    <div ref={containerRef} hidden={hidden}>
      ...
      <div className={classNames('fullscreen', styles.root)}>
        ...
        <div className={classNames(styles.content, !worldData && styles.content_loading)}>
          <Tabs
            tabs={Object.keys(providers)}
            disabledTabs={disabledProviders}
          ...
          >
            {providerActions && (
              ...
              >
                <span style={{ fontSize: 9, marginRight: 3 }}>Actions: </span>{' '}
                {Object.entries(providerActions).map(([label, action]) =>
                  typeof action === 'function' ? (
                    <Button key={label} onClick={action} style={{ width: 100 }}>
                      {label}
                    </Button>
                  ) : (
                    <Fragment key={label}>{action}</Fragment>
                  )
                )}
              </div>
            )}
              ...
              (() => {
                ...
                return Object.entries(groups).map(([groupName, worlds]) => (
                  <React.Fragment key={groupName}>
                    <GroupHeader name={groupName} count={worlds.length} expanded={expandedGroups[groupName] ?? true} onToggle={() => toggleGroup(groupName)} />
                    {(expandedGroups[groupName] ?? true) &&
                      worlds.map(({ name, size, detail, ...rest }, index) => {
                        const key = name
                        return (
                          <World
                            data-key={key}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minWidth: 400, paddingBottom: 3, alignItems: 'center' }}>
          {firstRowChildrenOverride || (
            <div>
              <Button rootRef={firstButton} disabled={!focusedWorld} onClick={() => onWorldAction('load', focusedWorld)}>
                Load World
              </Button>
              <Button onClick={() => onGeneralAction('create')} disabled={isReadonly}>
                Create New World
              </Button>
            </div>
          )}
          ...
          >
            {serversLayout ? (
              <Button style={{ width: 100 }} disabled={!focusedWorld || lockedEditing} onClick={() => onWorldAction('edit', focusedWorld)}>
                Edit
              </Button>
            ) : (
              <Button style={{ width: 100 }} disabled={!focusedWorld} onClick={() => onWorldAction('export', focusedWorld)}>
                Export
              </Button>
            )}
            <Button style={{ width: 100 }} disabled={!focusedWorld || lockedEditing} onClick={() => onWorldAction('delete', focusedWorld)}>
              Delete
            </Button>
            {serversLayout ? (
              <Button style={{ width: 100 }} onClick={() => onGeneralAction('create')} disabled={lockedEditing}>
                Add
              </Button>
            ) : (
              <Button style={{ width: 100 }} onClick={() => onWorldAction('edit', focusedWorld)} disabled>
                Edit
              </Button>
            )}
            <Button style={{ width: 100 }} onClick={() => onGeneralAction('cancel')}>
              Cancel
            </Button>
          </div>
        </div>
