...
const ChatProviderBase = () => {
  ...
  const { currentTouch: usingTouch, disconnectedCleanup } = useSnapshot(miscUiState)
  const {
    chatSelect,
    messagesLimit,
    chatOpacity,
  ...
  useEffect(() => {
    bot.addListener('message', (jsonMsg, position) => {
      ...
      if (promptKind && serverKey && savedPassword && (promptKind === 'login' || promptKind === 'either') && !isLoginMonitorActive()) {
        ...
        promptKind &&
        serverKey &&
        (promptKind === 'changepassword' || promptKind === 'unregister') &&
        loginPromptDebouncer.current.shouldTrigger(serverKey)
      ) {
        if (options.autoOpenAuthModal) {
          void showAutoFillLoginModal({ mode: promptKind, serverIp: serverKey, username: bot.username, prefilledPassword: findServerPassword() }).then(
            result => {
        ...
        serverKey &&
        !savedPassword &&
        (promptKind === 'login' || promptKind === 'register' || promptKind === 'either') &&
        loginPromptDebouncer.current.shouldTrigger(serverKey)
      ) {
        if (options.autoOpenAuthModal) {
          const mode = promptKind === 'either' ? 'login' : promptKind
          void showAutoFillLoginModal({ mode, serverIp: serverKey, username: bot.username }).then(result => {
            if (!result?.password) return
          ...
          const extra = promptKind === 'either' ? [makeExtra('login'), { text: ' / ' }, makeExtra('register')] : [makeExtra(promptKind)]
  ...
  const disabledReason = disconnectedCleanup
            ...
            if (chatVanillaRestrictions && !miscUiState.flyingSquid) {
              ...
              if (!validation.valid) {
                const choice = await showOptionsModal(
                  `Can't send invalid characters to vanilla server (${validation.invalid?.join(', ')}). You can use them only in command blocks.`,
                  ['Remove Them & Send']
                )
                if (!choice) return
