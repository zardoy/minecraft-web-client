...
export const readWorlds = (abortController: AbortController) => {
  ...
  ;(async () => {
    ...
    try {
      ...
      const newMappedWorlds = (
        await Promise.allSettled(
          worlds.map(async folder => {
...
const Inner = () => {
  ...
  const providerActions =
    loggedIn && selectedProvider === 'google' && isGoogleProviderReady && !selectedGoogleId
      ? {
          'Select Folder': selectGoogleFolder
        }
      : selectedProvider === 'google'
        ? isGoogleProviderReady
          ? loggedIn
            ? {
                'Log Out'() {
                ...
                async [`Read Only: ${googleDriveReadonly ? 'ON' : 'OFF'}`]() {
                  if (googleProviderState.readonlyMode) {
                    const choice = await showOptionsModal(
                      '[Unstable Feature] Enabling world save might corrupt your worlds, eg remove entities (note: you can always restore previous version of files in Drive)',
                      ['Continue']
                    )
    ...
    <Singleplayer
      ...
      onWorldAction={async (action, worldName) => {
        ...
        if (action === 'export') {
          ...
          const selectedVariant = haveDirectoryPicker()
