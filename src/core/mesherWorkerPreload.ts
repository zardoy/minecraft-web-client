 ...
 */
export async function preloadMesherWorkerScript(opts?: { fetchTimeoutMs?: number; pingTimeoutMs?: number }): Promise<void> {
  ...
  try {
    ...
    throw new MesherWorkerPreloadError(`Mesher script fetch failed (network): ${err?.message ?? e ?? 'unknown error'}. URL: ${scriptUrl}`, {
      phase: 'fetch',
      code: 'network',
