  ...
  source: {
    ...
    define: {
      ...
      'process.env.GITHUB_URL': JSON.stringify(
        `https://github.com/${process.env.GITHUB_REPOSITORY || `${process.env.VERCEL_GIT_REPO_OWNER}/${process.env.VERCEL_GIT_REPO_SLUG}` || githubRepositoryFallback}`
      ),
    ...
    pluginTypedCSSModules(),
    {
      ...
      setup(build: RsbuildPluginAPI) {
        const prep = async () => {
          ...
          if (
            !fs.existsSync('./generated/minecraft-data-optimized.json') ||
            !fs.existsSync('./generated/mc-assets-compressed.js') ||
            require('./generated/minecraft-data-optimized.json').versionKey !== require('minecraft-data/package.json').version
          ) {
          ...
          if (
            !fs.existsSync('./generated/latestBlockCollisionsShapes.json') ||
            require('./generated/latestBlockCollisionsShapes.json').versionKey !== require('minecraft-data/package.json').version
          ) {
